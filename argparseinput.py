...
def arginput():
    ...
    parser.add_argument("-spacing", type=int, action="store", metavar="Distance between probes", default=2)
    ...
    parser.add_argument("-mask", type = str, action = "store", metavar = "Mask repetitive sequences. Requires RepeatMasker. Not functional at the moment. T=True, F=False", default= "F")
    ...
    parser.add_argument("-low_Noff", type = int, action = "store", metavar = "Allowed number of probes with the same offtarget gene match in the same probeset",default=3)
    parser.add_argument("-medium_Noff", type = int, action = "store", metavar = "Allowed number of probes with the same offtarget gene match in the same probeset",default=7)
    parser.add_argument("-high_Noff", type = int, action = "store", metavar = "Allowed number of probes with the same offtarget gene match in the same probeset",default=14)
    parser.add_argument("-db_species", type = str, action = "store", metavar = "Species name. Should be supported by pyensembl",default=None)
    parser.add_argument("-padlock", type = str, action = "store", metavar = "Start site to retrieve probes. T=True, F=False", default= "F")
    parser.add_argument("-probe_type", type = str, action = "store", metavar = "Sequence order forms in format for `twist`, `opool` or `opool_amp`. Twist = Fw primer-readout 1:3 - Probe - Readout 4:6 - Rv primer. Opool = Probe - Readout 1:6. Opool_amp = Fw primer - Probe - Readout 1:6 - Rv primer", default= "twist")
    parser.add_argument("-min_probes", type = int, action = "store", metavar = "Minimum probes to activate high overlapping mode", default= 20)
    parser.add_argument("-max_probes_overlapping", type = int, action = "store", metavar = "maximum number of probes allowed to overlap", default= 20)
    parser.add_argument("-medium_overlap", type = int, action = "store", metavar = "maximum number of nucleotides that are allowed to overlap in medium overlapping mode", default= 2)
    parser.add_argument("-max_overlap", type = int, action = "store", metavar = "maximum number of nucleotides that are allowed to overlap in high overlapping mode", default= 10)
    ...
    parser.add_argument("-reference_sequence", type = str, action = "store", metavar = "Filename of fasta file with reference transcripts of each genes", default = "None")
